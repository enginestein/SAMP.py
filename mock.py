...
class MockSocket(asyncio.DatagramProtocol):
    responses = {
        ...
        b'SAMP\x7f\x00\x00\x01a\x1ex\x08\x00password\x11\x00banip 192.168.1.1': [b'\x1f\x00IP 192.168.1.1 has been banned.', None],
        ...
        b'SAMP\x7f\x00\x00\x01a\x1ex\x08\x00password\r\x00playertimeout': [b'\x1c\x00playertimeout = 10000  (int)', None],
        b'SAMP\x7f\x00\x00\x01a\x1ex\x08\x00password\r\x00rcon_password': [b'$\x00rcon_password = "password"  (string)', None],
        b'SAMP\x7f\x00\x00\x01a\x1ex\x08\x00password\x06\x00weburl': [b'#\x00weburl = "localhost:8000"  (string)', None],
        ...
        b'SAMP\x7f\x00\x00\x01a\x1ex\x08\x00password\x18\x00unloadfs WeatherStreamer': [b".\x00  Filterscript 'WeatherStreamer.amx' unloaded.", None],
    ...
    def sendto(self, data, addr=None):
        """
        Intercepts outgoing datagrams, looks up the canned response(s), and
        pushes them directly into the receive queue so that recvfrom() returns
        the right data without any real network I/O.
        """
        self.request = data
        self.counter = 0

        try:
            value = self.responses[data]
        except KeyError:
            return
        ...
        if isinstance(value, list):
            for frame in value:
                if frame is not None:
                    self._recv_queue.put_nowait(self.response_prefix + frame)
        else:
            self._recv_queue.put_nowait(self.response_prefix + value)
